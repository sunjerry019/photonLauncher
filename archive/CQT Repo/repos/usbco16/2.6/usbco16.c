/* usbco16.c:  This is the device driver for the usb/co16 adapter.
...
#include "usbco16_io.h"    /* contains all the ioctls */
...
#define USBDEV_NAME "usbco16"   /* used everywhere... */
#define USB_VENDOR_ID_CYPRESS 0x04b4
#define USB_DEVICE_ID 0x1237
...
static int usbdev_mmap(struct file * file, struct vm_area_struct *vma) {
    ...
    //printk("usbco16 mmap successful.\n"); /* for debugging...*/
    return 0;
}
/*************************************************************************/
...
static int usbdev_flat_open(struct inode *inode, struct file *filp) {
    ...
    if (err) {
      printk("Opening error point 1: err=%d\n",err);
      cp->iocard_opened = 0;
...
static long usbdev_flat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
	...
	case SendDac: case Timed_Inhibit:/* 16 bit argument */
	    data[3]=(arg>>8) & 0xff; /* MSbyte */
	    len++; chksum+=data[3];
	case Autoflush: /* one-byte parameter urbs */
	case RequestStatus:
	case Set_PortA:
	    data[2]=arg & 0xff; /* LSbyte */
	    len++; chksum+=data[2];
	case Reset_Hardware: case InitDac: /* no arguments */
	case Readback_PortA: case TimeToGo:
	case Set_Inhibitline: case Reset_Inhibitline:
	case Set_MasterReset: case Clear_MasterReset:
	case Initialize_FIFO: case Stop_nicely:
	case StartTransfer: case ManualFlush:
        case GetVersion:
	    ...
	    if (err) {
	      printk("error in sending command cmd: %08x; err=%d\n",cmd,err);
	      return err; /* return error number or 0 */
	    }
	    /* eventually return values */
	    if (cmd == RequestStatus || cmd==Readback_PortA ||
		cmd == TimeToGo || cmd == GetVersion) {
		/* read response */
		err=usb_bulk_msg(cp->dev, cp->inpipe1, cp->dummybuf,
				 5, &atrf, 1*HZ);
		if (err) {
		  printk("response in reading: %d\n",err); 
		  return err;
		}
		if (cmd==RequestStatus) cp->dummybuf[1]=0;
		/* get 16 bit value */
		return (unsigned char)cp->dummybuf[0] + 
...
static struct file_operations usbdev_simple_fops = {
    ...
    /* transition to newer kernel version */
...
static char classname[]="usbco16_%d";


/* when using the usb major device number */
static struct usb_class_driver co16class = { 
    name: classname,
...
static int usbdev_init_one(struct usb_interface *intf, const struct usb_device_id *id ) {
    ...
    retval=usb_register_dev(intf, &co16class);
 ...
 out1:
    usb_deregister_dev(intf, &co16class);
 out2:
...
static void usbdev_remove_one(struct usb_interface *interface) {
	...
	if (cp->transfers_running) {
	  printk("unplugging point 0; cp: %p, running:%d\n",cp, cp->transfers_running);
	    cp->transfers_running=0;
	    /* stop DMA machinery */
    ...
    usb_deregister_dev(interface, &co16class);

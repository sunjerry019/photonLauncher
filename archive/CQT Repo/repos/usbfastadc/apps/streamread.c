/* This code is a modified version of the oscilloscope code that only outputs
   unprocessed binary data in the generic form from the converter.


   current options: 
   streamread [-d <devicefile>]
              [-n <numberofsamples>]
	      [-m <inputmode>]
	      [-O ]
	      [-p <prescaler>]
  ...
  History:
  8.8.2014          first version, starts working. don't choose the 
                    -p option too small try -p 1 or -p 2 first.

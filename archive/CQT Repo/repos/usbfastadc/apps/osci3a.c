...
int main(int argc, char *argv[]){
    ...
    int bufferoverrun=0; /* indicates a buffer turnover */
    int tmp;
...
#define TRIG_DONE 4             /* we have everything */
    ...
    //fprintf(stderr,"# point 1, command: %lx\n",WRITE_CPLD_LONG);
    //usleep(100000);
    ...
    bufferoverrun=0; /* normal operation */
    do { /* polling until number has been reached */
      if (!bufferoverrun) usleep(POLLTIME);
      v=ioctl(handle, Get_transferredbytes);
      if (v<0) return -emsg(16);
      if (!bufferoverrun) totalacquisitiontime += POLLTIME;
      ...
      samples = v/sizeof(int); /* number of total 16bit entries */
      //printf("this time %d samples.\n",samples-visited_already);
      tmp=visited_already/samplenum;
      if (samples/samplenum > tmp) {
	/* we now have a buffer turnover to handle */
	samples=(tmp+1)*samplenum;
	bufferoverrun=1;
	// printf("or @ samples: %d\n",samples);
      } else {
	bufferoverrun=0;
      }
      testindex = &intarray[(visited_already + trigchan) % samplenum];
	...
	if (trigdel < 0)  {
	  visited_already = -trigdel*wordspersample;
	}
	testindex = &intarray[(visited_already + trigchan) % samplenum];
	...
	if (trigpol<0) { /* we need to be above triglev2 */
	  for (i=visited_already; i<samples; i+= wordspersample, testindex += wordspersample)
	    if ((*testindex & trigmask) >= triglev2) break;
	...
	if (trigpol<0) { /* we need to be below triglev1 */
	  for (i=visited_already; i<samples; i+= wordspersample, testindex += wordspersample)
	    if ((*testindex & trigmask) <= triglev1) break;

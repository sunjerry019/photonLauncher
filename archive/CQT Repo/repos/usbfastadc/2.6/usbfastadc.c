/* usbfastadc.c   -  USB device driver for fast adc unit, with jtag loader

  Copyright (C) 2010-15 Christian Kurtsiefer <christian.kurtsiefer@gmail.com>

  This driver provides a simple interface to the control commands passed onto
  the device via the EP1 interface. Commands can be found in "usbfastadc_io.h"
  for the adc operation, and in "usbprog_io.h" for the programming section.
  Commands are implemented mostly with ioctl() calls.
  ...
  At the moment, the code was mostly taken from the timestamp driver. A revamp
  of the driver was necessary when it turned out that the older driver had
  issues on newer 64bit kernel versions. Most likely the problem was silly
  ioctl() command definition macros.


  status: 28.11.12chk  first code
  7.8.2014chk rewritten to work with 64bit OS
...
#include "usbfastadc_io.h"    /* contains ioctls for adc operation */
...
MODULE_DESCRIPTION("Experimental driver for fast ADC card\n");
MODULE_LICENSE("GPL");  

#define USBDEV_NAME "usbfastadc"   /* used everywhere... */
#define USB_VENDOR_ID_CYPRESS 0x04b4
#define USB_DEVICE_ID 0x1245
...
static int usbdev_mmap(struct file * file, struct vm_area_struct *vma) {
    ...
    printk("usbfastadc mmap successful.\n"); /* for debugging...*/
    return 0;
}
/*************************************************************************/
...
static long usbdev_flat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    unsigned char data[256]; // = cp->dummybuf;
	...
	*/
	...
	return cp->errstat;
	break;
    case Set_transferlength:
	if (arg<0) return 0;
	if (arg > cp->minmempiece) {
	    cp->initial_transferlength = cp->minmempiece;
	} else {
	    cp->initial_transferlength = arg;
	}
	return cp->initial_transferlength;
	break;
    ...
    case START_STREAM: case STOP_STREAM: case START_QSTREAM:
    case GETRDYLINESTAT:
	...
	data[len-1]=chksum;
	//printk("send usb1 msg (cmd=0x%x):\n",cmd);
	//for (i=0; i<data[0];i++) printk(" %02x",data[i]);
	//printk("\n");
	/* wait for 1 sec */
	localerr=usb_bulk_msg(cp->dev, cp->outpipe1, data, len, &atrf, 1*HZ);
	//printk("retval: %d\n",err);
    ...
    if ((cmd == GETRDYLINESTAT) || (cmd == GETBYTECOUNT) ||(cmd == GET_TCB)) {
	//printk( "exciting point 1: cmd=0x%x \n", cmd);
	localerr=usb_bulk_msg(cp->dev, cp->inpipe1, data,
			      64, &atrf, 1*HZ);
    ...
    if ( (cmd == START_STREAM) || 
	 (cmd == START_LIMITED) ) {
	//printk("start stream\n");
	localerr=usb_bulk_msg(cp->dev, cp->inpipe1, data, 64, &atrf, 1*HZ);
	if (localerr) {
	    return localerr;
	}
...
static char classname[]="usbfastadc%d";


/* when using the usb major device number */
static struct usb_class_driver fastadcclass = { 
    name: classname,
...
static int usbdev_init_one(struct usb_interface *intf, const struct usb_device_id *id ) {
    ...
    retval=usb_register_dev(intf, &fastadcclass);
 ...
 out1:
    usb_deregister_dev(intf, &fastadcclass);
 out2:
...
static void usbdev_remove_one(struct usb_interface *interface) {
    ...
    usb_deregister_dev(interface, &fastadcclass);

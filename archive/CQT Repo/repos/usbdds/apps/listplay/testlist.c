/* program example to test elements of the listplay mode;
   ...
   This code generates a table of configuration entries for a linear 
   chirp/amplitude span, mainly for testing of the listplay mode 
   on a single channel (0).

   Once this code has run, the elements are stepped through with pulses at
   the P3 (P1 as of board rev 2) input, and may be reset to the head of the
   list with a pulse on P2 (P0). 
...
int main(int argc, char *argv[]){
    ...
    for (i=0;i<NUMBER_OF_ENTRIES; i++) {
      ...
      freq = 10000000 + i*10000000; /* steps of 10 MHz */
      amp = 10.*i;

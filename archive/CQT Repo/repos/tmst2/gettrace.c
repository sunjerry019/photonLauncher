/* This code is a modified version of the oscilloscope code that only outputs
   unprocessed binary data in the generic form from the converter. Adapted to
   work with the timestamp test board.


   current options: 
   gettrace   [-d <devicefile>]
              [-n <numberofsamples>]
	      [-m <inputmode>]
	      [-O ]
              [-f filename]
  ...
  OPTIONS:
   ...
   -s <phaseshift>   set the phase shift of the pulse with respect to
                     conversion clock 
   -h                hold mode, does not switch off adc
   -i                inverts phase of clock4, shifting it by 25ns
  ...
  History:           first code transfer 20141127chk
...
#define INITIAL_TRANSFERLENGTH (1<<14)
...
char clock_data[] = {
  ...
  /* Output configuration channels 2,1 */
  6, 0x01, 0x9e,
  0x00, 0x31, 0x02, /* ch 2: div by 50 ->20 MHz, LVDS3.5mA */
  0x80, 0x31, 0x01, /* ch 1: div by 50 ->20 MHz ,PECL true phase, 1/4 periodex */

  /* Output configuration channels 5, 4 */
  6, 0x01, 0xb3,
  0x00, 0x18, 0x08, /* ch 5: /25 ->40MHz, CMOS, true phase (IFCLK)*/
  0x00, 0x31, 0x08, /* ch 4: /50 ->20MHz, CMOS, true phase (aux clk)*/
...
/* set single register; the phase value is in reg ... */
char phaseshift_data[] = {
  /* Output configuration channel 4: phase shift, sign */
  3, 0x01, 0xb0,
  0x00, 0x31, 0x08, /* ch 4: /48 ->20MHz, CMOS, true phase (aux clk)*/
...
/* code to set the phase shift for clock4 (in basic units */
int phase_shift(int handle, int shift, int sign) {
  int retval;
  phaseshift_data[3]=(shift & 0x3f)<<2; 
  if (sign) {
      phaseshift_data[5]=0x88;
  } else {
      phaseshift_data[5]=0x08;
  }
  retval=SPI_configure(handle, phaseshift_data, CLOCKCHIP_WRITE);
  if (retval) return retval; /* some error */
  return retval;
}
...
int main(int argc, char *argv[]){
    ...
    int phaseshift=0; /* clock4 shift */
    int sign=0; /* clock4 polarity */
    int holdmode=0;
    ...
    while ((opt=getopt(argc, argv, "d:n:m:Of:qs:hi")) != EOF) {
       switch (opt) {
	 ...
	 if ((inputmode<0) || (inputmode >2)) return -emsg(15);
	 break;
	 ...
	 quietoption=1;
	 break;
       case 's': /* phase shift setting */
         if (sscanf(optarg,"%d",&phaseshift)!=1) return -emsg(20);
	 break;
       case 'h': /* holdmode */
	 holdmode=1;
	 break;
       case 'i': /* set inverse sign */
	 sign=1;
	 break;
       }
    }
    ...
    retval=ioctl(handle, WRITE_CPLD,
		 0x0c | modeword[inputmode] );
    ...
    /* set phase shifter */
    if (phase_shift(handle, phaseshift, sign)) return -emsg(20);    

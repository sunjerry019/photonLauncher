...
#define COUNTINGTIMESPAN 12000000 /* the count monitor duration in 125psec */
...
int main (int argc, char *argv[]) {
    ...
    int storagesize=STORAGE_SIZE; /* store the max number of events timingch1 and timingch2 can store. */
    ...
    patternmask |= channel1;
    patternmask |= channel2;
    /* printf("channel1: %i, channel2: %i, patternmask %i\n",(int)channel1, (int)channel2, (int)patternmask); */
    do{ 
        retval=fread(inbuf, sizeof(struct rawevent), 1, inhandle); /*read the next event and define it to be zeroth event */
/*	printf("dv :%u\n",  inbuf[index].dv); */
        pattern = (inbuf[index].dv & patternmask);
/*	 printf("pattern:%llu\n", pattern); */
    }while(!pattern);
...
/*             printf("pattern: %llu\n", pattern); */
            ...
            if (pli[ip]<0) 
                continue;   /*only take good patterns */
...
/*                 printf("%d, %d\n", steventno1, storagesize); */
/*                 printf("%d, %d\n", steventno2, signalthreshold); */
...
/*         -totalnoise1*totalnoise2*signalbins*timespan/((double)noisebins*noisebins*countingtimespan); */
    ss=(totalsignal1/(double)signalbins-totalnoise1/(double)noisebins)*(totalsignal2/signalbins-totalnoise2/(double)noisebins)*timespan*signalbins/((double)countingtimespan);
...
/*     bgg2N=((totalsignal1/signalbinsd-totalnoise1/noisebinsd)*totalnoise2/noisebinsd+(totalsignal2/signalbinsd-totalnoise2/noisebinsd)*totalnoise1/noisebinsd+totalnoise1/noisebinsd*totalnoise2/noisebinsd)/(totalsignal1/signalbinsd*totalsignal2/signalbinsd); */
    ...
    /* output histogram */
    fprintf(outhandle,"bin No.\tdelay\tun_g2a\tn_g2a\tn_g2b\tun_g2b\n");//\tco_g2\n");
    for (i=0;i<timebinnumber;i++) {
        center = ((i*timespan)+timespan/2.0f)*0.125;
        fprintf(outhandle,"%d\t%0.3f\t%d\t%.3lf\t%.3lf\t%.3lf\n"/* ,\t%.3lf\n" */,i,center,histo[i],(float)histo[i]/Ntotal, histoN[i]/blockno, histoN[i]/* , (histo[i]-bgcont)/ss */);
    }

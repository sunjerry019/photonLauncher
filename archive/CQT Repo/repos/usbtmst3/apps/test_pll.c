/* program to test the proper operation of the clockchip on the intermediaty
   time stamp card.
...
#define DEFAULT_DEVICENAME "/dev/ioboards/timestamp0"
...
char Channel1_config[6] = { /* this programs the channel 1 related registers
  ...
  0x01        /* 0x199: LVPECL (8mA), obey sync */
...
char ConfigureStatus1[4] = { /* setting of the FS1 mux to PLL1 locked */
  3,
  0x02, 0x31,
  0x02  /* pll1 is locked */
};
...
int main(int argc, char *argv[]) {
  ...
  printf("point 2A\n");
  if (ioctl(handle, SubmitPLLtext, ConfigureStatus1)) return -emsg(4);

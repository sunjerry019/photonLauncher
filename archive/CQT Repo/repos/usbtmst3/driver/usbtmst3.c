/* usbtmst3.c:  This is the device driver for an update of the old
   timestamp card. Code in transit, based on the google code driver
   as of 6.6.2014
...
#define USBDEV_NAME "timestamp"   /* used everywhere... */
#define USB_VENDOR_ID_CYPRESS 0x04b4
#define USB_DEVICE_ID 0x1233
...
static long usbdev_flat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  struct cardinfo *cp = (struct cardinfo *)filp->private_data;
  unsigned char data[64]; /* send stuff */
  ...
  case  WriteDelayChip:/* 16 bit argument */
  ...
  /* code to send a whole command chain to the PLL chip. */

/* program to read a jedec file, and generate a jtag command set to program a
   ...
   Strategy:
    ...
    -j nodename       specify the node name of the jtag interpreter
    -C target         specify the chip type. Possible options can be queried
                      if targer is a question mark
    -v                verbosity level adjustment. Add more v's for more details
...
#define DEFAULTTARGET "/dev/iboards/usbtmst4"

/* command definitions for XO2 JTAG engine */
...
char *errormessage[] = {
  ...
  "Cannot parse jtag node name", /* 20 */
  "Cannot parse target chip name",
  "Unknown device. Try -C ? to list known chip",
  "No jedec content found",
  "Cannot open target node",
  "Error writing to USB device", /* 25 */
  "Error in delay time calculation",
  "Illegal choice of JTAG state",
  "Wrong shift mode",
  "Bit chain length error", 
  "Error in bit chain comparison", /* 30 */
  "Bit chain mismatch",
  "Device ID mismatch",
  "Mismatch in bypass test near RAM clear",
...
int populate_jed_structure(FILE *sourcehandle, struct jedecinfo *js) {
    ...
    /* consistency checks */
    if (parsemode<10) return -emsg(23);
...
/*---------------------------------------------------------------------*/
/* Code that deals with chip parameter loading */
extern struct deviceproperties *devlist[];

/* find a device index via name. Returns an index */
int get_Device_by_name(char *name) {
    int i;
    struct deviceproperties * d;
    i=0;
    if (strcmp("?", name)==0) { /* we need to show the list */
	printf("List of known targets: \n");
	for (;;) {
	    d=devlist[i];
	    if (d==NULL) break;
	    printf(" %s\n",d->dname);
	...
	d=devlist[i];
	if (d==NULL) break;
	if (strncmp(d->dname, name, 20)==0) return i;
	i++;
    }
...
int main (int argc, char *argv[]) {
    ...
    char targetnodename[200] = DEFAULTTARGET;
    char targetdevicename[200] = "";

    struct deviceproperties *targetdevice;
    ...
    /* parse options */
    while ((opt=getopt(argc, argv, "f:j:vC:")) != EOF) {
	switch (opt) {
	case 'f': /* input file name */
	    if (1!=sscanf(optarg,"%199s", sourcefilename)) return -emsg(1);
	    break;
	case 'j': /* jtag node name */
	    if (1!=sscanf(optarg,"%199s", targetnodename)) return -emsg(20);
	    break;
	case 'v': /* verbosity level */
	    verbosity++;
	    break;
	case 'C': /* chip name */
	    if (1!=sscanf(optarg,"%199s", targetdevicename)) return -emsg(21);
	    break;
	}
    }
    ...
    if (verbosity>0) {
	printf("Jedec file successfully parsed.\n\n");
	if (verbosity>1) {
	    printf(" Pin number: %d\n", jed.pinnumber);
    ...
    if (targetdevicename[0]) 
	retval=get_Device_by_name(targetdevicename);
    if (retval==-1) return -emsg(22); /* does not exist */
    if (retval==-2) return 0; /* showed list */
    targetdevice=devlist[retval];  
  
    if (verbosity >0) printf("Selected device: %s\n",
			     targetdevice->dname);

    /* open device handle */
    targethandle=open(targetnodename, O_RDWR);
    if (targethandle==-1) return -emsg(24);

    /* initialize JTAG interface */
    retval=JTAG_init(targethandle);
    if (retval) return -emsg(retval);
    
    retval=Set_ENDDR(targethandle, JTAG_STATE_DRPAUSE);
    if (retval) return -emsg(retval);
    retval=Set_ENDIR(targethandle, JTAG_STATE_IRPAUSE);
    if (retval) return -emsg(retval);
    retval=Go_State(targethandle, JTAG_STATE_IDLE);
    if (retval) return -emsg(retval);
    
    /* check ID */
    retval = sendreceive_32(targethandle, IDCODE_PUB, 
			    0, targetdevice->idcode, 0XFFFFFFFF);
    if (retval==31) { /* mismatch error */
	return -emsg(32); /* be specific that there is a device id mismatch */
    }
    if (retval) return -emsg(retval); /* otherwise general error msg */

    /* we have a valid device */
    if (verbosity>0) {
	printf("Device successfully identified with code 0x%08x over jtag.\n",
	       targetdevice->idcode);
    }

    /* isolate pins from chip */
    if (verbosity>1) printf("Isolating boundary scan elements.\n");
    retval=Bscan_208(targethandle, PRELOAD);
    if (retval) return -emsg(retval);
    
    /* erase configuration RAM */
    if (verbosity>0) printf("Clearing RAM.....");
    retval=send_8(targethandle, ISC_ENABLE, 0x00);
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* wait 1 msec */
    if (retval) return -emsg(retval);
    retval=send_8(targethandle, ISC_ERASE, 0x01); /* Erase RAM */
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 1.0); /* wait 1 sec */
    if (retval) return -emsg(retval);
    retval=Bypasstest(targethandle);
    if (retval==31) return -emsg(33); if (retval) return -emsg(retval);
    if (verbosity>0) printf("done.\n");
  
    /* Switch on flash prog mode */
    if (verbosity>0) printf("Switching on flash mode...");
    retval=send_8(targethandle, ISC_ENABLE, 0x08);
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* wait 1 msec */
    if (retval) return -emsg(retval);
    if (verbosity>0) printf("done.\n");

    /* Check fuses - seems more a status register check for failure */
    if (verbosity>0) printf("Checking ISC status...");
    retval=sendreceive_32(targethandle, LSC_READ_STATUS, 
			  0, 0, 0x00020000); /* Check for fail */
    if (retval) return -emsg(retval);
    retval=sendreceive_32(targethandle, LSC_READ_STATUS, 
			  0, 0, 0x00010000); /* Check for busy */
    if (retval) return -emsg(retval);
    if (verbosity>0) printf("done.\n");
#define eraseflash
#ifdef eraseflash
    /* Erase flash */
    if (verbosity>0) printf("Erasing Flash.....");fflush(stdout);
    retval=send_8(targethandle, ISC_ERASE, 0x0e); /* all three areas */
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.); /* two cycles */
    if (retval) return -emsg(retval);
    do {/* waiting loop for not busy */
	retval=LSC_Busy_test(targethandle);
    } while (retval==31); 
    if (retval) return -emsg(retval); /* in case something went wrong */
    retval=sendreceive_32(targethandle, LSC_READ_STATUS, 
			  0, 0, 0x00020000); /* Check for fail */
    if (retval) return -emsg(retval);
    if (verbosity>0) printf("done.\n");
...
#ifdef flashprogram    
    /* Program CFG flash */
    if (verbosity>0) printf("Programming config flash.....");fflush(stdout);
    retval=send_8(targethandle, LSC_INIT_ADDRESS, 0x04); /* What data?? */
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.01); /* two cycles */
    if (retval) return -emsg(retval);
    /* loop through all rows of 128 bit */
    for (fuseindex=0; fuseindex<targetdevice->cfgsize; fuseindex+=128) {
	if (verbosity>1) {
	    printf("*"); /* a star per row */
	    fflush(stdout);
	}
	retval=send_128bit(targethandle, LSC_PROG_INCR_NV,
			   &jed.fusefield[fuseindex/8]);
	if (retval) return -emsg(retval); /* some error occured */
	retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.); /* two cycles */
	if (retval) return -emsg(retval);
...
#ifdef programufm   
    /* Program UFM */
    if (verbosity>0) printf("Programming UFM.....");fflush(stdout);
    retval=send_cmdonly(targethandle, LSC_INIT_ADDR_UFM); /* no data? */
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    if (retval) return -emsg(retval);
    /* loop through all rows of 128 bit */
    for (fuseindex=targetdevice->cfgsize; 
	 fuseindex<targetdevice->fusenumber; fuseindex+=128) {
	if (verbosity>1) {
	    printf("*"); /* a star per row */
	    fflush(stdout);
	}
...
#define proguser
#ifdef proguser
    /* Program User code */
    if (verbosity>0) printf("Programming Usercode.....");fflush(stdout);
    u=jed.ufield[0]+(jed.ufield[1]<<8)+
	(jed.ufield[2]<<16)+(jed.ufield[3]<<24); /* u32 value */
    retval=sendreceive_32(targethandle, ISC_PROGRAM_USERCODE, u, 0, 0);
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.); /* two cycles */
    if (retval) return -emsg(retval);
    do {/* waiting loop for not busy */
    ...
    retval=sendreceive_32(targethandle, LSC_READ_STATUS, 
			  0, 0, 0x00020000); /* Check for fail */
    if (retval) return -emsg(retval); /* or actually to go cleanup */

    /* verify user code */
    retval=sendreceive_32(targethandle, USERCODE, 
			  0, u, 0xffffffff); /* Check for fail */
    if (retval) return -emsg(retval); /* or actually to go cleanup */    
    if (verbosity>0) printf("done.\n");
#endif
    ...
    /* Verify cfg */
    if (verbosity>0) printf("Verifying config flash.....");fflush(stdout);
    retval=send_8(targethandle, LSC_INIT_ADDRESS, 0x04); /* what data?*/
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    if (retval) return -emsg(retval);
    retval=send_cmdonly(targethandle, LSC_READ_INCR_NV);
    if (retval) return -emsg(retval);
    //retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    //if (retval) return -emsg(retval);
    ...
    //retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    //if (retval) return -emsg(retval);
    /* loop through first row of 128 bit */
    //retval=verify_128bit(targethandle, &jed.fusefield[0]);
    //if (retval) return -emsg(retval);
    /* loop through all rows of 128 bit */
    for (fuseindex=0; fuseindex< targetdevice->cfgsize; fuseindex+=128) {
	retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two tck */
	if (retval) return -emsg(retval);
	if (verbosity>1) {
	    printf("*"); /* a star per row */
	    fflush(stdout);
	}
	printf("index: %d\n",fuseindex); fflush(stdout);
	retval=verify_128bit(targethandle, &jed.fusefield[fuseindex/8]);
	if (retval) return -emsg(retval); /* some error occured */
    }
    if (verbosity>0) printf("done.\n");
#endif
    ...
    /* Verify ufm */
    if (verbosity>0) printf("Verifying user flash.....");
    retval=send_cmdonly(targethandle, LSC_INIT_ADDR_UFM); /* What data?? */
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    if (retval) return -emsg(retval);
    retval=send_cmdonly(targethandle, LSC_READ_INCR_NV);
	...
	retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two tck */
	if (retval) return -emsg(retval);
	if (verbosity>1) printf("*"); /* a star per row */
	retval=verify_128bit(targethandle, &jed.fusefield[fuseindex/8]);
	if (retval) return -emsg(retval); /* some error occured */
    }
    if (verbosity>0) printf("done.\n");
    ...
    if (verbosity>0) printf("Programming feature row.....");fflush(stdout);
    retval=send_8(targethandle, LSC_INIT_ADDRESS, 0x02); /* What data?? */
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    if (retval) return -emsg(retval);
    retval=send_64bit(targethandle, LSC_PROG_FEATURE,
			   &jed.efield[0]);
    if (retval) return -emsg(retval); /* some error occured */
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0); /* two cycles */
    if (retval) return -emsg(retval);
    ...
    retval=send_cmdonly(targethandle, LSC_READ_FEATURE);
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    if (retval) return -emsg(retval);
    retval=verify_64bit(targethandle, &jed.efield[0]);
    if (retval) return -emsg(retval);
    if (verbosity>0) printf("done.\n");
   
    /* Program FEA bits */
    if (verbosity>0) printf("Programming FEA bits.....");fflush(stdout);
    u=jed.efield[8]+(jed.efield[9]<<8);
    printf("fea: %04x\n",u); 
    retval=send_16(targethandle, LSC_PROG_FEABITS, u); 
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.); /* two cycles */
    if (retval) return -emsg(retval);
    do {/* waiting loop for not busy */
    ...
    retval=send_cmdonly(targethandle, LSC_READ_FEABITS);
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.001); /* two cycles */
    if (retval) return -emsg(retval);
    retval=verify_16(targethandle, 0,u,0xfff2);
    if (retval) return -emsg(retval);
    retval=sendreceive_32(targethandle, LSC_READ_STATUS, 
			  0, 0, 0x00030000); /* Check for fail/busy */
    if (retval) return -emsg(retval); /* or actually to go cleanup */
    if (verbosity>0) printf("done.\n");


    /* Program done */
    if (verbosity>0) printf("Programming done bit.....");
    retval=send_cmdonly(targethandle, ISC_PROGRAM_DONE);
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.); /* two cycles */
    if (retval) return -emsg(retval);
    do {/* waiting loop for not busy */
    ...
    retval=sendreceive_32(targethandle, LSC_READ_STATUS, 
			  0, 0, 0x00100000); /* Check for "programmed" bit */
    if (retval) return -emsg(retval); /* or actually to go cleanup */
    if (verbosity>0) printf("done.\n");
    ...
    /* Exit programming */
    if(verbosity>0) printf("Exiting programming...");
    retval=send_cmdonly(targethandle, ISC_DISABLE);
    if (retval) return -emsg(retval);
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 1.0); /* wait 1 sec */
    if (retval) return -emsg(retval);
    retval=send_cmdonly(targethandle, BYPASS);
    if (retval) return -emsg(retval);   
    retval=runtest(targethandle, JTAG_STATE_IDLE, 2, 0.1); /* wait 100 msec */
    if (retval) return -emsg(retval);
    if(verbosity>0) printf("done.\n");
